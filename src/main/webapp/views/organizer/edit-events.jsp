...
<body class="bg-base-200 min-h-screen flex flex-col">
	...
	<header
		...
		<h1 class="text-lg font-bold text-center flex-grow">Edit Event</h1>
	...
	<div class="container mx-auto p-4 flex-grow">
		...
		<%
            // Fetch the eventID from the request parameter
		...
		<!-- Form to edit event -->
		<form id="editEventForm" action="EditEventServlet" method="POST"
			...
			<div class="mb-4">
				<label for="eventTime" class="label">Event Time</label> <input
					...
					class="input input-bordered w-full"
					value="<%= event.getEventTime().toString().substring(0, 5) %>"
					required>
			</div>
			...
			<div class="mb-4">
				<label for="eventLocation" class="label">Event Location</label> <input
					...
					value="<%= event.getEventLocation() %>" required>
			</div>
			...
			<div class="mb-4">
				<label for="eventPrice" class="label">Event Price</label> <input
					...
					value="<%= event.getEventPrice() %>" min="0" required>
			</div>
			...
			<div class="mb-4">
				<label for="maxParticipant" class="label">Maximum
					...
					value="<%= event.getMaxParticipant() %>" min="1" required>
			</div>
			...
			<div class="mb-4">
				...
				<textarea id="description" name="description"
					class="input input-bordered w-full"
					placeholder="Enter the description" maxlength="1000" required><%= event.getDescription() %></textarea>
			</div>

			<!-- Event Status -->
			<div class="mb-4">
				<label for="eventStatus" class="label">Event Status</label> <select
					id="eventStatus" name="eventStatus"
					class="select select-bordered w-full" required>
					<option value="Upcoming"
						<%= event.getEventStatus().equals("Upcoming") ? "selected" : "" %>>Upcoming</option>
					<option value="Ongoing"
						<%= event.getEventStatus().equals("Ongoing") ? "selected" : "" %>>Ongoing</option>
					<option value="Ended"
						<%= event.getEventStatus().equals("Ended") ? "selected" : "" %>>Ended</option>
				</select>
			</div>

			<!-- Organizer ID (hidden field) -->
			<input type="hidden" id="organiserID" name="organiserID"
				value="<%= event.getOrganiserID() %>">
